...
export default function AddReminderScreen() {
  ...
  useEffect(() => {
    getUserMedicines().then((meds) => {
      setMedicines(meds);
      // Pre-select if navigated with params
      if (params.medicineId) {
        ...
        if (found) setSelectedMed(found);
          ...
          // Medicine not in saved list yet, create a placeholder
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // ← plug in your API: POST /api/reminders
  ...
  if (success) {
    return (
      <SafeAreaView style={[styles.safe, { alignItems: 'center', justifyContent: 'center', gap: 20, padding: 32 }]} edges={['top', 'bottom']}>
        ...
        <Pressable onPress={() => { setSuccess(false); setSelectedMed(null); setTime(new Date()); }}>
      ...
      <ScrollView contentContainerStyle={styles.page} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.pillGrid}>
          {TIME_SLOTS.map((slot) => (
            <Pressable
              key={slot}
              style={[styles.pill, time === slot && styles.pillActive]}
              onPress={() => setTime(slot)}
            >
              <Ionicons
                name="time-outline"
                size={13}
                color={time === slot ? Colors.primary : '#94A3B8'}
              />
              <Text style={[styles.pillText, time === slot && styles.pillTextActive]}>
                {slot}
              </Text>
            </Pressable>
          ))}
