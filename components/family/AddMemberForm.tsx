 ...
 * DOB uses DatePickerField (Expo native calendar).
 */
...
export function AddMemberForm({ onContinue }: Props) {
  ...
  const handleContinue = () => {
    ...
    // Format to YYYY-MM-DD for the API; empty string if not selected
    const date_of_birth = dob
      ? `${dob.getFullYear()}-${String(dob.getMonth() + 1).padStart(2,'0')}-${String(dob.getDate()).padStart(2,'0')}`
      : '';
    onContinue({ relationship: rel, full_name: name, phone, date_of_birth });
  };
          ...
          <Pressable key={r} style={styles.relChip} onPress={() => setRel(r)}>
            <View style={[styles.relIcon, r === rel && styles.relIconSel]}>
              <Ionicons
                name="person-outline"
                size={18}
                color={r === rel ? '#fff' : Colors.textMuted}
              />
            </View>
